---
name: duolingo-french-cards-skill
description: Convert Duolingo "Words" screenshots into French vocabulary study-card artifacts. Use when the user asks to extract French words from Duolingo screenshots, generate `duolingo-words.csv`, enrich words with French IPA, examples, usage notes, English and Chinese translations in `ipa-examples.csv`, or render `[P#H1#word]` TXT cards such as `study-cards.txt`; also use for the local Duolingo-momo-word screenshot-to-card workflow.
---
...
3. Extract visible French word rows into `outputs/duolingo-words.csv`.
4. Read `references/enrichment-guidelines.md`, then create `outputs/ipa-examples.csv` with IPA, a natural example, usage notes, English translation, and Chinese translation.
5. Read `references/card-format.md`, then render `outputs/study-cards.txt`. Prefer the bundled script:
...
- `outputs/duolingo-words.csv`: columns `法语单词,英文翻译`.
- `outputs/ipa-examples.csv`: columns `法语单词,法语英标,例句,词性/用法,英文翻译,中文翻译`.
- `outputs/study-cards.txt`: one `[P#H1#...]` card per enriched CSV row, ending each card with `*****`.
